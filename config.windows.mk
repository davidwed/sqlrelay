...
# connections
CONNECTIONSALLTARGETS = @ALLDB2@ @ALLFIREBIRD@ @ALLMYSQL@ @ALLODBC@ @ALLORACLE@ @ALLPOSTGRESQL@ @ALLSQLITE@ @ALLSYBASE@ all-router
CONNECTIONSINSTALLTARGETS = @INSTALLDB2@ @INSTALLFIREBIRD@ @INSTALLMYSQL@ @INSTALLODBC@ @INSTALLORACLE@ @INSTALLPOSTGRESQL@ @INSTALLSQLITE@ @INSTALLSYBASE@ installdll-router
...
# bench
BENCHCPPFLAGS = $(BASECPPFLAGS) $(RUDIMENTSINCLUDES) /I$(top_builddir)\src\api\c++ $(DB2INCLUDES) $(FIREBIRDINCLUDES) $(FREETDSINCLUDES) $(MYSQLINCLUDES) $(ORACLEINCLUDES) $(POSTGRESQLINCLUDES) $(SQLITEINCLUDES) $(SYBASEINCLUDES)
