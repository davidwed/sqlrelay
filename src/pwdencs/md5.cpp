...
class MD5
{
	...
	private:
		...
		void Encode (unsigned char*, uint32_t*, uint64_t);
		void Decode (uint32_t*, const unsigned char*, uint64_t);
		void MD5_memcpy (POINTER, POINTER, uint64_t);
		void MD5_memset (POINTER, int, uint64_t);
	...
	public:
		...
		static char *MD5Sum(const unsigned char *, uint64_t);
...
/*
	 ...
	 context.
*/
void MD5::MD5Update (MD5_CONTEXT *context, const unsigned char *input, uint64_t inputLen)
{
	  uint64_t i, index, partLen;
 ...
 */
void MD5::MD5Transform (uint32_t state[4], const unsigned char block[64])
{
...
/* 
 * Encodes input  into output (unsigned char). Assumes len is
 * a multiple of 4.
 */
void MD5::Encode (unsigned char *output, uint32_t *input, uint64_t len)
{
	uint64_t i, j;
...
/*
 * Decodes input (unsigned char) into output . Assumes len is
 * a multiple of 4.
 */
void MD5::Decode (uint32_t *output, const unsigned char *input, uint64_t len)
{
	  uint64_t i, j;
 ...
 */
void MD5::MD5_memcpy (POINTER output, POINTER input, uint64_t len)
{
	uint64_t i;
 ...
 */
void MD5::MD5_memset (POINTER output,int value,uint64_t len)
{
	uint64_t i;
	for (i = 0; i < len; i++)
...
char *MD5::MD5Sum(const unsigned char *data, uint64_t size) {
	...
	md5.MD5Final(buff,&ctx);

	//converte the hash to a string and return it
	return md5.MD5ConvertToAscii(buff);
}
