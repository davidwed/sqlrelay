...
void sqlrlistener::clientSession(filedescriptor *clientsock,
	...
	if (pvt->_dynamicscaling) {
		incrementConnectedClientCount(thr);
	}
...
void sqlrlistener::incrementConnectedClientCount(thread *thr) {
	...
	if (pvt->_semset->supportsTimedSemaphoreOperations()) {
		...
		bool	timeout=false;
		if (!semWait(7,thr,true,&timeout)) {

			if (timeout) {
				raiseDebugMessageEvent(
					"timed out waiting for the scaler");
			} else {
				raiseDebugMessageEvent(
					"failed waiting for the scaler");
